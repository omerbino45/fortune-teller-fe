  ...
  if (!token) {
    return (
      <div className="app-shell flex flex-col min-h-dvh items-center justify-center px-6 text-center">
        ...
        <h1 className="text-xl font-bold text-tx1 mb-2">קישור לא תקין</h1>
        <button onClick={() => navigate('/login')} className="btn-primary px-8 py-3 text-sm mt-4">חזרה לכניסה</button>
      </div>
    )
  }
  ...
  return (
    <div className="app-shell flex flex-col min-h-dvh">
      ...
      <div className="flex-1 px-6 pt-8">
        <p className="text-tx2 text-sm mb-6 leading-relaxed font-light">הזן סיסמה חדשה לחשבון שלך.</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          {[
            { label: 'סיסמה חדשה', name: 'newPassword' as const },
            { label: 'אימות סיסמה', name: 'confirmPassword' as const },
          ].map(({ label, name }) => (
            <div key={name}>
              ...
              <div className="relative">
                ...
                <input {...register(name)} type="password" dir="ltr" className="input-dark !pr-10" placeholder="••••••••" />
              </div>
